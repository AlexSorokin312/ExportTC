...
namespace ExportTC.Model
{
    public class InitialDataSetter : IInitialDataSetter
    {
        ...
        // Метод для отложенной инициализации, если путь меняется позже
        private void EnsureReaderInitialized()
        {
            if (_excelReader == null && !string.IsNullOrEmpty(_initialData.ExcelFile))
            {
        ...
        {
            EnsureReaderInitialized();  // Инициализация только при необходимости
